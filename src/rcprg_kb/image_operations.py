...
class OperationEnlarge:
    ...
    def compute(self, in_data):
        ...
        if self.border_value is None:
            img_corr = cv2.filter2D(
                img, -1, self.kernel, borderType=cv2.BORDER_REPLICATE)
        else:
            img_border = cv2.copyMakeBorder(
                img, 1, 1, 1, 1, borderType=cv2.BORDER_CONSTANT, value=self.border_value)
            img_corr = cv2.filter2D(
                img_border, -1, self.kernel, borderType=cv2.BORDER_REPLICATE)[1:-1, 1:-1]
