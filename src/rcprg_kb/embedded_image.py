...
def overlayImage(src, dst, pos, dst_shape=None):
    assert (not dst is None and dst_shape is None) or (
        dst is None and not dst_shape is None)
    src_sx = src.shape[1]
    src_sy = src.shape[0]
    ...
    if dst is None:
        ...
        result = np.copy(dst)
    if len(src.shape) == 3:
        result[dst_y_min:dst_y_max, dst_x_min:dst_x_max,
               :] = src[src_y_min:src_y_max, src_x_min:src_x_max, :]
    else:
        result[dst_y_min:dst_y_max,
               dst_x_min:dst_x_max] = src[src_y_min:src_y_max, src_x_min:src_x_max]
...
class EmbeddedImage:
    ...
    def getScaled(self, scale):
        img = cv2.resize(self.__img__, None, fx=scale,
    ...
    def getTransformed(self, tf):
        rot = tf[1]
        px_center = (
            int(-self.__origin__[0]/self.__res__), int(-self.__origin__[1]/self.__res__))
        ...
        origin_new = (self.__origin__[0] + tf[0]
                      [0], self.__origin__[1] + tf[0][1])
...
def addI(im1, im2):
    ...
    im2_mv = overlayImage(im2.getImage(), None, pos_px,
                          dst_shape=im1.getImage().shape)
...
def drawLine(im, pt1, pt2, color, thickness):
    ...
    cv2.line(im.getImage(), int_tuple(pt1_px),
             int_tuple(pt2_px), color, thickness=thickness)
...
def drawCircle(im, pt, radius, color, thickness):
    ...
    cv2.circle(im.getImage(), int_tuple(pt_px),
...
def test_EmbeddedImage(output_path):
    ...
    origin = (-4, -2)
    points = [(0.5, 0.5), (0.7, 2), (2, 2),
              (2.5, 1.5), (-origin[0], -origin[1])]
